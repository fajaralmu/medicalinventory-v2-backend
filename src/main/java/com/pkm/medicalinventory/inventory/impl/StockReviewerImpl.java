...
@Service
public class StockReviewerImpl implements StockReviewer {
	@Autowired
	...
	public static Integer expiryStatus(Date kadaluarsa) {
		Integer sts = 1;
		Date d = new Date();
	...
	private static PeriodicReviewResult calculatePeriodicReview(
		Double demand,
		Double stdev,
		double time,
		double lTime,
	...
	public PeriodicReviewResult periodicReview(
		int stock,
		List<InventoryData> usageData,
		boolean normalityTest
	) {
		Integer[] usage = new Integer[usageData.size()];

		for (int i = 0; i < usageData.size(); i++) {
			usageData.get(i).checkNulls();
			usage[i] = usageData.get(i).getTotalItemsSum();
		}

		if (normalityTest) {
			if (usage.length > 30 && maxInt(usage) > 10) {
				if (!isNormalChiSQR(usage, 0.05))
					return PeriodicReviewResult.builder().description("Failed isNormalChiSQR").build();
			} else {
				if (!isNormalKolmogorov(usage, 0.05))
					return PeriodicReviewResult.builder().description("Failed isNormalKolmogorov").build();
			}
		}

		Double[] meanstd = getMeanStDeviation(usage, false);
		Double stdev = meanstd[1];
		Double mean = meanstd[0];
		
		System.out.println(stdev + " " + mean);
		
		ConfigurationModel cfg = inventoryConfigurationService.getConfiguration();
		return calculatePeriodicReview(
			mean,
			stdev,
			cfg.getCycleTime(),
			cfg.getLeadTime(),
...
//	}
	...
	public static Integer[] order(Integer[] list_int) {
	...
	public static Double[] getMeanStDeviation(Integer[] list, boolean sample) {
		int s = sample ? 1 : 0;
	...
	private boolean isNormalKolmogorov(Integer[] list_in, double signifikansi) {
		Integer[] list = order(list_in);
		Double[] meanstd = getMeanStDeviation(list, true);
