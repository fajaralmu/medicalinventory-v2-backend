...
public class TransactionReceiptBuilder extends BaseReportBuilder {
	...
	public TransactionReceiptBuilder(Transaction transaction, HealthCenter mainLocation, String fileName) {
		super(fileName);
	...
	public WritableReport build() {
		final HealthCenter location = this.mainLocation;

		Consumer<Document> writeAction = (doc) -> {
			try {
				PdfPTable table = new PdfPTable(8);
				// pt.setTotalWidth(800);

				table.setTotalWidth(new float[] { 50, // no
						200, // name
						170, // exp date
						170, // batch
						120, // record id
				...
				if (transaction.getType().equals(TransactionType.TRANS_OUT)
						|| transaction.getType().equals(TransactionType.TRANS_OUT_TO_WAREHOUSE)) {
				...
				table.addCell(noHead);
				table.addCell(namaHead);
				table.addCell(expDate);
				table.addCell(batch);
				table.addCell(idStok);
				table.addCell(jmlHead);
				table.addCell(hargaSatuanHead);
				table.addCell(totalHargaItemHead);
				...
				for (ProductFlow ao : transaction.getProductFlows()) {
					...
					table.addCell(nama);
					table.addCell(exp);
					table.addCell(itemBatch);
					table.addCell(ID_STOK);
					table.addCell(jml);
					table.addCell(hargaItem);
					table.addCell(totalHargaItem);
				...
				table.addCell(emptyCell);
				table.addCell(emptyCell);
				table.addCell(jmlTotal);
				table.addCell(emptyCell);
				table.addCell(jmlHargaTotal);
		...
		return new TransRecepitReport(document, fileName, writeAction);
	}
