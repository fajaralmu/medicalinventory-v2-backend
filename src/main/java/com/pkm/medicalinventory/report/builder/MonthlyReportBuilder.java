...
public class MonthlyReportBuilder extends BaseReportBuilder {
	...
	public WritableReport build() {
		...
		for (Integer day = 1; day <= 31; day++) {
			...
			loop: for (Transaction transaction : transactions) {
				...
				String destinationName = transaction.getCustomer() != null ? transaction.getCustomer().getName()
						: transaction.getHealthCenterDestination().getName();
	...
	private static boolean transactionMatch(Transaction transaction) {
		...
		if (TransactionType.TRANS_OUT.equals(transaction.getType()) && transaction.getCustomer() != null) {
			return true;
		}
		return false;
	}
	...
	private List<Transaction> getTransactionsToDestination(HealthCenter location) {
		...
		return transactionsOneMonth.stream().filter(t -> {
	...
	private void writeConsumtionSummary() {
		...
		List<Transaction> toCustomers = transactionsOneMonth.stream()
				.filter(t -> TransactionType.TRANS_OUT.equals(t.getType())).collect(Collectors.toList());
